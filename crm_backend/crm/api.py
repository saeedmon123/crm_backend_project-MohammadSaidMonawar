...
def get_customer_profiles(request, customer_id: int):
    try:
        ...
        customer_data = {
            ...
            'last_contacted': customer.last_contacted.isoformat(),
        }
    ...
    try:
        ...
        for product_id, quantity in zip(product_ids, quantities):
            ...
            if product.quantity_available < quantity:
                return JsonResponse({'error': f'The product with the name {product.name} currently has only {product.quantity_available} units available in stock.'}, status=404)
          ...
          # Return response
            ...
            if UseLoyalty == True:
                ...
                if RedeemPoints >= min_point_to_redeem:
                    ...
                    return JsonResponse({"message":"you don't reach the minimum point to redeem {min_point_to_redeem}"})
    ...
    try:
        with transaction.atomic():
            ...
            if promotion:
                ...
                promotion.save()
                Promotion.check_usage_limits(promotion)
...
def create_interaction(request, participant_type: ParticipantTypeChoices, participant_id: int, interaction_type: InteractionTypeChoices, interaction_details: str, outcome: str, responsible_user: str, follow_up_required: bool):
